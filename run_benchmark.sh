...
# run_benchmark.sh - Reusable benchmark runner for VoxelMap (ROS1)
...
if [ -z "$CONFIG_NAME" ] || [ -z "$INPUT_BAG_FILE" ]; then usage; fi

CONFIG_FILE="${SCRIPT_DIR}/configs/${CONFIG_NAME}.env"
if [ ! -f "$CONFIG_FILE" ]; then echo "Error: Config not found: $CONFIG_FILE"; usage; fi
...
echo "=========================================="
echo "Running VoxelMap benchmark (ROS1)"
...
IMAGE="${VOXELMAP_IMAGE:-ghcr.io/mapshd/voxelmap2hdmapping:latest}"
...
docker run --rm -t \
    ...
    bash -lc "
      ...
      if [ -f /test_ws/devel/setup.bash ]; then source /test_ws/devel/setup.bash
      elif [ -f /test_ws/install/setup.bash ]; then source /test_ws/install/setup.bash; fi
      ...
      roslaunch ${VOXELMAP_LAUNCH} >/tmp/voxelmap.log 2>&1 &
      LAUNCH_PID=\$!
      ...
      rosrun voxel-to-hdmapping listener \"\${REC_BAG}\" \"\${OUT_DIR}/hdmapping\"
    "
